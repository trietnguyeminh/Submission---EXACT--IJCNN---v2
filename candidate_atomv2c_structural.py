...
def parse_premise(p):
    ...
    # [atomv2c] existential frame: 'there exists at least one X who P' -> predicate P only
    me=re.match(r'^\s*there\s+(?:exist[s]?|is|are)\s+(?:at\s+least\s+one|at\s+least|some|one|a|an)\s+[a-zA-Z]+\s+(?:who|that|which|whom)\s+(.+)$',s,re.I)
    if me:
        lit=to_literal(me.group(1));
        return ('fact',lit) if lit else None
    # [atomv2c] 'if X, Y' comma rule (no explicit 'then')
    mcf=re.match(r'^\s*if\b(.+?),\s*(.+)$',s,re.I)
    if mcf and not re.search(r'\bthen\b',s,re.I):
        ante=re.split(r'\band\b',mcf.group(1),flags=re.I)
        lits=[to_literal(x) for x in ante]; lits=[l for l in lits if l]
        cons=re.sub(r'^\s*(?:the\s+[a-zA-Z]+|they|it|he|she)\s+(?:will|shall|would|is|are|do|does)?\s*','',mcf.group(2),flags=re.I)
        con=to_literal(cons)
        if con and lits: return ('rule',lits,con)
    # [atomv2c] universal fact: 'Every/All <noun> P' -> predicate P only
    mu=re.match(r'^\s*(?:every|all|each)\s+[a-zA-Z]+\s+(.+)$',s,re.I)
    if mu:
        lit=to_literal(mu.group(1));
        return ('fact',lit) if lit else None
    lit=to_literal(s)
    return ('fact',lit) if lit else None
